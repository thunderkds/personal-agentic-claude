...
**T032 merged** (2026-07-17, worktree `agent-a60c669423604cff2` — note: the sub-agent's own `isolation: "worktree"` created a *second*, separate worktree/branch instead of reusing the one `common-infrastructure` had already set up at `pac-T032`/`feat/setup-direct-install`; that worktree/branch was orphaned and cleaned up unused): `setup.sh` rewritten — sources `lib/harness-fetch.sh`, `check_target_is_git_repo()` runs before any write, always-overwrite real copies via `harness_copy_manifest` (**backup-then-overwrite since T112**), new `.claude/harness-lock.json` writer with **per-file** (not per-directory) content hashes, sha256/shasum content-only (chmod-independent). Stage 4 review: 0 P0/P1, 2 P2 accepted as-is (JSON path-key escaping doesn't cover all control chars — low risk, repo-controlled paths; `resolve_repo_url` gained a `SUPERVISOR_REPO` override for offline test fixtures, a deliberate deviation from the guide's "keep unchanged" note, accepted since additive-only). Independent `verify` (not the author's `tests/test_setup.sh`) confirmed real end-to-end behavior plus a probe not in the test suite: a stale symlink at a MANIFEST path (old-model leftover) is correctly overwritten to a real directory by `setup.sh` without touching the symlink's former target — confirms `setup.sh`'s "always overwrite" contract is distinct from `update.sh`'s (T033) "refuse on symlink" contract, not a contradiction between them. **T112 revisited exactly this probe and changed the answer**: a symlink at a MANIFEST path is now moved to `<path>.bak` as the link itself (target untouched), not overwritten in place. `install_pack()`/`packs/` confirmed untouched.
...
**T105 merged** (2026-09-06, `fix/t105-shellcheck-green`): CI's "Shellcheck install scripts" step was exiting 1 on four info/style findings — `scripts/smoke-install.sh:36` SC2329, `tests/test_harness_projection.sh:299` SC2181, `:385`/`:386` SC2012. Two distinct root causes, not one. **SC2329**: line 35 already carried a correct `# shellcheck disable=SC2317` for its trap-invoked `cleanup`, but shellcheck split that case into a new SC2329, so the directive suppressed a dead check while the live one landed unsuppressed — CI went red with **no commit to the repo**, apt's shellcheck having rolled forward under it. Fixed by listing both codes. **SC2181/SC2012**: long-standing checks against T097 code last touched 2026-08-31; fixed by code change (test `run_setup`'s outcome directly; `ls -1` → `find -mindepth 1 -maxdepth 1 -printf '%f\n'`). New `tests/test_shellcheck_clean.sh` mirrors the CI step locally and fails loudly when shellcheck is absent rather than skipping-as-pass (per the "invisible off switch" learning). **Version-pinning shellcheck in `ci.yml` was offered and the user declined** — CI stays on apt so new checks keep surfacing, accepting that this class of breakage recurs; `ci.yml` was therefore untouched. Stage 4: 0 P0, 0 P1, 1 P2, 2 P3. `/verify` PASS driven at the running scripts, not the suite: CI's command exits 0, both empty-`--harness` forms still refuse by name and write nothing, `test_harness_projection.sh` 41/0 on **both** `main` and the branch. **Merged as-is by user decision with 2 open follow-ups** in `tasks/TASK_REVIEW_T105.md`: the mirror test hardcodes its file list (demonstrated during `/verify` to diverge from CI), and `find -printf` is a GNU extension adding a non-POSIX dependency to a previously portable suite. Unconfirmed and labelled as such throughout: *when* CI first went red — `gh` is unauthenticated here, so "red since T097" is inference from file dates, not run history.
**Files**: scripts/smoke-install.sh, tests/test_harness_projection.sh, tests/test_shellcheck_clean.sh, tasks/TASK_GUIDE_T105.md, tasks/TASK_REVIEW_T105.md

**T106 merged** (2026-09-06, `docs/t106-readme-v2`): the README had never caught up with v2.0.0 — `git diff v1..HEAD -- README.md` was 19 insertions across a release `RUNBOOK.md:208` marks BREAKING. T101 (`b6b51a0`) had already reconciled the *mechanics* correctly (`--harness`, plain-root canon, symlinks, update re-derivation), so four gaps survived it: no version string anywhere in 73 lines; the Codex install invited without mentioning that four skills are skipped there; "a framework for Claude Code" framing; and a stale "once T084's deploy is run" TODO. Fixed all four. Site URL `https://personal-agentic-claude.vercel.app/` supplied by the user and verified live before use (HTTP 200, served HTML byte-identical to `site/index.html`), and **recorded in `RUNBOOK.md`'s landing-site section** — its absence from the repo is what forced the round-trip. New `tests/test_readme_current.sh` derives the current version from RUNBOOK's release log and the cap + oversize set from `lib/harness-fetch.sh` and the skills tree; nothing hardcoded. Stage 4: 0 P0, **1 P1** — the test guarded only one drift direction, demonstrated by shrinking `write-better-skill` under the cap and watching it still pass; returned to the agent and fixed, now bidirectional plus a cap-agreement check (13 assertions). P2 fixed; P3 (whole-file grep scoping) declined with reason. `/verify` PASS driven **at the installer**, not the suite — the README asserts what `setup.sh` does, so a real Codex install was run: installer output named exactly the four skills with matching byte sizes, 26 skills landed, all four absent on disk; `HARNESS_SKILL_BODY_CAP=0` → both oversize skills present (so "currently affected" is accurate); a Claude install → 0 warnings, all 30 skills (the cap is correctly Codex-scoped).
**Follow-on**: T107 registered for the "Easy Kit" rename — the user's naming decision, out of T106's scope because choosing the canonical name is a product call. The `personal-agentic-claude` slug is explicitly NOT renamed: it is the install path, and changing it breaks every existing install's update path.
**Files**: README.md, RUNBOOK.md, tests/test_readme_current.sh, tasks/TASK_GUIDE_T106.md, tasks/TASK_REVIEW_T106.md
...
### 2026-09-18 — Install is backup-then-overwrite, and `harness_backup_path` is public for T114 (T112)
**Decision**: `setup.sh` no longer destroys a project's own files. `harness_backup_path <src> <dst>`
in `lib/harness-fetch.sh` moves a differing pre-existing `<dst>` to the first free `<dst>.bak`,
`.bak.1`, `.bak.2`… before the kit's copy lands, and is called from both `harness_copy_manifest`
and `install_claude`. Rules, in the order they are evaluated: a missing destination does nothing;
a destination identical to the kit's (files by `cmp -s`, directories by `diff -rq`, never for
symlinks) does nothing and prints nothing; anything else — file, directory, symlink, or type
mismatch — is moved and named in a `[warn]` line. An existing backup is **never** overwritten. A
symlink is moved as the link itself, its target untouched. A failed `mv` returns 1 after an
`[error]`, and the caller must not replace the destination — checked explicitly rather than left to
`set -e`, because a caller in an `if`/`&&` context runs without it. A backup at `.claude/hooks`
additionally warns that `settings.json` entries pointing at the user's own hooks must be repointed.
**The function is public on purpose**: T114's "Reinstall (backs up your edits)" calls it instead of
re-implementing it. Unlike `harness_install_canon_symlinks`, an existing `.bak` does not fail the
run — the next free number keeps both backups.
**Why**: ADR-0002 "No silent loss". The pre-T112 `rm -rf` at `lib/harness-fetch.sh:152` deleted a
project's `templates/`, `docs/claude-md/`, `CLAUDE.md`, `AGENTS.md` and `.claude/hooks` on a first
install and still printed `Setup complete.` — reproduced live against `main` at Stage 5, not
inferred. ADR-0002 chose backup-and-proceed over an interactive per-file choice; the plan screen
that asks first is T114's job, not the installer's.
**Known limitation, carried as a follow-up, not a defect**: the abort message says "nothing was
replaced" while MANIFEST-earlier paths already were — see `learnings.md`, same date. Data is safe;
