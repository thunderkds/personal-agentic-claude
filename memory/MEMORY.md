...
- ~~step_limit/trace false-positive~~ **FIXED by T043** — both hooks now attribute structurally; the bracket-glob-the-ID workaround is retired. NEW consequence: a `Bash` command is never attributed, so export `CLAUDE_ACTIVE_TASK=Txxx` when running tests or the merge gate finds no trace record
