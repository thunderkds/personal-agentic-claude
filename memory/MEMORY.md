...
> **token-audit RETIRED 2026-08-05 (T030 / DDR-0002)**: the instrument never captured `/cost`
> ground truth in either window, so DDR-0001's ≥20%/<5% $/task criteria were unanswerable from any
> data collected — a second instrument failure, this time on the metric itself. Both candidate
> refactors DDR-0001 gatekept had already shipped for unrelated reasons (T049 CLAUDE.md 565→198,
> T029 slim-skills), so there was nothing left to decide even with real numbers. DDR-0002 supersedes
> DDR-0001; no new window opens. `reports/token-audit_*.md` retained as historical record only.
...
- [T044 Stage 4 CLOSED, not yet merged](decisions.md) — merge gate now matches a runner **anchored at a command head** (quoted spans = data), `post_agent_move_to_review.py` deliberately writes nothing (no completion event carries task identity), `craft-spawn-prompt` emits the `CLAUDE_ACTIVE_TASK=Txxx` instruction that keeps the tightened gate from failing closed on honest tasks. code-review 0 P0 / 1 P1 fixed / 2 P3; security-review **PASS, 0 actionable** (manual — the built-in diffs the *checked-out* branch, so it reports a false clean when run from main against another branch); both HTML reports saved. **Fully merged to main 2026-07-31** (docs half PR #55 `a6dd66b`, feature half `de120da`); Stage 5 verify re-run from main: 109 passed + smoke PASS. Shipped with a live defect in its own AC7 → **T047**
...
- [Stage 2 planning T047, 2026-07-31](decisions.md) — repair trace attribution's **channel**, not T044's logic (which is correct + reviewed); channel choice deliberately left to the implementing agent (3 candidates, none pre-selected), "no channel exists" sanctioned as a valid outcome; T040 re-blocked → DDR-0001's 2nd window can't open until this lands
...
- [T047 merged + verified; follow-up T048 also merged](decisions.md) — attribution via `.claude/hooks/.state/active_task` **works end-to-end** (first correct Bash-tool-call attribution ever; gate returns True). BUT arming it fails 9 pre-existing tests → **T048 (P0)**. **Open**: shared `.state` root races across worktrees
...
- [T047 Stage 4 closed: active-task state file](decisions.md) — attribution now travels via `.claude/hooks/.state/active_task` (ID+timestamp, stale after 6h), resolved off `$CLAUDE_PROJECT_DIR`; env slot kept but only settable before the session starts. **Open**: shared `.state` root races across concurrent worktrees
...
- ~~step_limit/trace false-positive~~ **FIXED by T043** — both hooks now attribute structurally; the bracket-glob-the-ID workaround is retired. Consequence: a `Bash` command is never attributed. ~~so export `CLAUDE_ACTIVE_TASK=Txxx` when running tests~~ **that workaround does not work** (2026-07-31) — the export can't reach the hook process; every Bash record lands in `_untagged.jsonl` and the merge gate fails closed. Being fixed by **T047**; until it lands there is no way for an agent to file an attributed trace record
