...
The fix is a channel `task_context.py` can actually observe: a plain state file, `.claude/hooks/.state/active_task`, written with a shell redirect from inside any `Bash` tool call. Include this line in every assembled prompt, with `Txxx` substituted:

> **Trace attribution**: before running any test or verification command, write the active-task state file so the trace hook can attribute your `Bash` calls: `mkdir -p .claude/hooks/.state && printf '%s\n%s\n' "Txxx" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > .claude/hooks/.state/active_task`. Do this once at the start of your session — it stays valid for `CLAUDE_ACTIVE_TASK_STATE_MAX_AGE_S` seconds (default 6h). Do **not** try `CLAUDE_ACTIVE_TASK=Txxx <cmd>` inside a `Bash` tool call — it does not reach the hook and will leave your evidence unattributed.
