...
- **Hot tier** (`memory/MEMORY.md`): ≤52,000 characters. Supervisor-curated index. One-line summaries + links to cold files. Passed to every sub-agent as a **path to read**, not pasted into the spawn prompt — the agent opens it itself as a mandatory startup step.
...
- **Diff-driven pass procedure**:
  ...
  5. Summarize new/changed entries as one-liners in `memory/MEMORY.md` (keep ≤52,000 characters total)
