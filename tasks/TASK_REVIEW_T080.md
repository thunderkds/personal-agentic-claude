...
| verify | ☐ N/A | Documentation-only task with no runtime surface to launch; `verify` is invoked by the Supervisor at Stage 5, not the implementing sub-agent (a sub-agent has no `Skill` tool) |
