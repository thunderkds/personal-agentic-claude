...
| verify | ☑ pass | **User-run `/verify` at Stage 5, 2026-08-18 — first pass FAIL, second pass PASS after two fixes.** Driven at two surfaces, not by replaying the suite. **Gate surface**: live file `measured=40536 entries=138 budget=45000 -> green`; pushed over with 20 realistic entries `-> measured=47267 -> RED`, so the lowered ratchet genuinely bites; and **the deliverable** — file cut to `measured=2438 entries=8`, ~17x below the old coupling's floor, and `test_ac10` **still passed** where the old loop raised `DID NOT RAISE` (confirmed twice, also at 2,766 chars). **Shipping surface — this is where the FAIL came from**: executing `setup.sh`'s heredoc into a temp project produced a seeded `MEMORY.md` stating **both** figures — line 3 `Max 45,000 characters — a ratchet`, line 22 `The enforced gate is the 50,000-character whole-file budget above`. Every downstream repo would have been scaffolded with a memory file contradicting itself about the one number this task exists to change, and it is invisible in-repo because the live `memory/MEMORY.md` carries no such comment block. `test_ac8` asserted the new figure was *present* and carried negative assertions for two other stale strings (`"200 lines"`, `"Injected in full"`) but not for the old budget — the one negative assertion that would have caught it was the only one missing. **Both fixed**: `setup.sh` line 367 corrected, and `test_ac8` now asserts *every* `\d{1,3},\d{3}` figure in the stub equals `HOT_TIER_CHAR_BUDGET`, written that way rather than `"50,000" not in stub` so the next ratchet cannot reintroduce the decay under a different number. **Mutation-controlled from two directions**: restoring the stale `50,000` → RED with `found ['45,000', '50,000'], expected only 45,000`; and mutating `HOT_TIER_CHAR_BUDGET` to `40_000` → also RED, proving the assertion tracks the constant rather than a hardcoded 45,000. Re-driven after the fix: both stub figures now read 45,000. Final: **649 passed**. |
