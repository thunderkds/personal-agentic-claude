...
| Review scope bounded to the change's blast radius | ✅ pass | **Stage 4 code-review** (Supervisor, 2026-07-31): scope = 4 changed files + the 2 direct callers of the entry point (`post_tool_trace.py`, `pre_agent_step_limit.py`), read but not modified. Phase 0.5 entry point `resolve_task_id` found in all 3 hook callers ✅ reachable. Result **0 P0 / 1 P1 (found + fixed at `41f4c14`) / 0 P2 / 0 P3**. The P1: `ACTIVE_TASK_MAX_AGE_S` was resolved with a bare `int(os.environ...)` at *import* time, below the never-raises contract — a value of `6h` raised `ValueError`, and since both callers guard the import with `except Exception`, attribution died **repo-wide** including the unrelated path-field slot. Reproduced empirically before fixing (a `Read` of this guide landed in `_untagged.jsonl`). Fix mutation-tested by the Supervisor: restoring the bare `int()` → RED on the new test only, reverted → 121 passed. **Stage 4 security-review**: manual + labelled (the built-in diffs the *checked-out* branch, which is not this one) — **PASS, 0 actionable**; injection suite against the task-ID validator (path traversal, null byte, `$(...)`, `;`-chaining, overflow) all rejected. **Reports**: `reports/code-review_feat-trace-attribution-channel_20260731T105322.html`, `reports/security-review_feat-trace-attribution-channel_20260731T105322.html`. **Supervisor independently reproduced** the implementer's boundary-straddling mutation (reverting `_resolve_root` to `__file__` → RED, reverted → green) rather than trusting the claim. Diff touches only `.claude/hooks/lib/task_context.py`, `.claude/hooks/tests/test_task_context.py`, `.claude/skills/craft-spawn-prompt/SKILL.md`, `tasks/TASK_GUIDE_T047.md`. `git diff --stat -- .claude/hooks/pre_bash_block_unsafe_merge.py .claude/hooks/pre_agent_validate_guide.py` is empty (AC9 held). |
