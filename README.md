...
| `memory/MEMORY.md` | *(per project)* Hot-tier memory index — ≤52,000 characters, referenced by path in every sub-agent spawn prompt and read by the agent |
...
```
memory/MEMORY.md          ← Hot tier (≤52,000 chars, referenced by path in spawn prompts)
...
- **Size gate.** The hot tier is capped at **52,000 characters** (not lines — a 200-line cap stayed green while the file grew 15.5% in characters). The budget is a ratchet: `/compact-memory` may lower it, nothing raises it.
