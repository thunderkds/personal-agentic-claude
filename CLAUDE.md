...
| `ui-test` | `.claude/skills/ui-test/SKILL.md` | Stage 5 `verify`: self-degrading UI test orchestration over the local `easy-ui-mcp` server — invoked by `qa-expert` only for tasks with a non-deleted "UI / Design Acceptance Criteria" section; skips cleanly if the server is down or the task has no UI. |
...
6. **UI tasks: all three design Evidence rows must be filled before Done or `ship`.** Any task that includes a UI component must have the "UI / Design Acceptance Criteria" section in its TASK_GUIDE completed, and all three Evidence rows — visual regression, design-system compliance, and responsiveness — must show a pass result with pasted evidence (or ☐ N/A with a written justification). A UI task with blank or unchecked design Evidence rows is **not done**, regardless of how logic tests look. For pure-backend tasks, delete the UI/Design AC section from the TASK_GUIDE and mark all three UI Evidence rows ☐ N/A. When available, MCP-produced evidence from the `ui-test` skill satisfies these three rows directly (screenshot, page-state, and per-viewport results mapped into the table); when `ui-test` is skipped or unavailable, the rows must still be filled with real evidence from manual or other verification — the gate's requirement is never waived.
